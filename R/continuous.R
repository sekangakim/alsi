## R/continuous.R  -  alsi package v0.2.0
## calsi_workflow(): continuous ALSI via ipsatized SVD. Ported from calsi.R.
## Adaptations:
##   ipsatize() removed; use .alsi_ipsatise(scale_sd=FALSE) from R/utils.R
##   summarise_matrix() -> .alsi_summarise_matrix() from R/utils.R
##   svd_ipsatized() -> .alsi_svd_ipsatized() (internal)
## ALSI formula: same as binary - variance-weighted L2 norm (unsigned).
## K-selection: svd_pa() using paran (with permutation fallback). K_override available.
...
#' @keywords internal
.alsi_svd_ipsatized <- function(X, K = NULL) {
  X <- as.matrix(X)
...
svd_pa <- function(data, 
  ...
  if (requireNamespace("paran", quietly = TRUE)) {
    ...
    X_std_ips <- .alsi_ipsatise(X_scaled)
...
svd_bootstrap <- function(data, 
  ...
  # Reference fit
  fit_ref <- .alsi_svd_ipsatized(X, K = K)
  ...
  # Summary statistics
  angles_summary <- .alsi_summarise_matrix(angles, probs = c(0.05, 0.95))
  tucker_summary <- .alsi_summarise_matrix(tucker, probs = c(0.05, 0.95))
...
calsi_workflow <- function(data,
  ...
  ## Build stability_table in the cross-pipeline agreed format
  ang_sum <- boot$angles_summary
  tuc_sum <- boot$tucker_summary
  eig_K   <- fit$eig[seq_len(K)]
  stab_tbl <- data.frame(
    Dimension         = seq_len(K),
    Eigenvalue        = round(eig_K,               6L),
    Cum_eigen_pct     = round(100 * cumsum(eig_K) / sum(eig_K), 1L),
    Angle_deg_median  = round(ang_sum[, "median"],  2L),
    Angle_deg_p5      = round(ang_sum[, "p5"],      2L),
    Angle_deg_p95     = round(ang_sum[, "p95"],     2L),
    Tucker_phi_median = round(tuc_sum[, "median"],  3L),
    Tucker_phi_p5     = round(tuc_sum[, "p5"],      3L),
    Tucker_phi_p95    = round(tuc_sum[, "p95"],     3L),
    Grade_Tucker      = .alsi_tucker_grade(tuc_sum[, "median"]),
    stringsAsFactors  = FALSE
  )

  ## PA table
  pa_eig <- pa$eig_obs
  pa_ref <- if (!is.null(pa$eig_adj)) pa$eig_adj else pa$eig_rand
  pa_tbl <- data.frame(
    Dimension    = seq_along(pa_eig),
    Observed     = round(pa_eig, 5L),
    Reference    = round(pa_ref, 5L),
    Exceeds_ref  = seq_along(pa_eig) <= K,
    stringsAsFactors = FALSE
  )

  out <- structure(list(
    ALSI_index      = calsi_obj$alpha,
    ALSI_z          = as.vector(scale(calsi_obj$alpha)),
    K_PA            = K,
    K_star          = K,
    Z_ref           = fit$F[, seq_len(K), drop = FALSE],
    lambda_rot      = eig_K,
...
#' @export
print.calsi_result <- function(x, ...) {
  cat("calsi_result (ipsatized SVD, variance-weighted L2 norm)\n")
  cat(sprintf("  K_PA = K* = %d  |  N = %d  |  method: %s\n",
              x$K_star, length(x$ALSI_index), x$pa$method))
  cat("\nStability table:\n")
  print(x$stability_table[, c("Dimension","Eigenvalue","Angle_deg_median",
                               "Tucker_phi_median","Grade_Tucker")],
        row.names = FALSE)
  cat("\nDomain contributions (%):\n")
  print(round(sort(x$domain_contrib, decreasing = TRUE), 2L))
  cat(sprintf("\ncALSI (raw, range [%.3f, %.3f]):\n",
              min(x$ALSI_index), max(x$ALSI_index)))
  print(round(summary(x$ALSI_index), 4L))
  invisible(x)
}
