## R/binary.R  -  alsi package v0.2.0
## alsi_workflow() binary ALSI via MCA. Ported from balsi.R (CRAN v0.1.3).
## Internal helpers renamed .alsi_*. summarise_matrix -> .alsi_summarise_matrix.
## ALSI = variance-weighted L2 norm (unsigned). K from PA only.
...
#' @keywords internal
.alsi_make_disjunctive <- function(X01) {
...
#' @keywords internal
.alsi_mca_indicator <- function(Xbin01) {
  # Validate and convert to binary
  Xbin01 <- as.data.frame(Xbin01)
  Xbin01[] <- lapply(Xbin01, .alsi_to01)
  ...
  Xd <- .alsi_make_disjunctive(Xbin01)
  M <- ncol(Xd)
...
mca_pa <- function(data, 
  ...
  dat <- if (is.character(data)) .alsi_read_xlsx(data) else as.data.frame(data)
  ...
  Xbin[] <- lapply(Xbin, .alsi_to01)
  Xbin <- Xbin[complete.cases(Xbin), , drop = FALSE]
  
  # Reference MCA
  fit0 <- .alsi_mca_indicator(Xbin)
  eig_obs <- fit0$eig
  Kmax <- length(eig_obs)
  ...
  for (b in seq_len(B)) {
    ...
    eig_perm[b, ] <- .alsi_mca_indicator(Xp)$eig
...
mca_bootstrap <- function(data, 
  ...
  # Reference fit
  fit_ref <- .alsi_mca_indicator(Xbin)
...
alsi_workflow <- function(data,
  ...
  pa_tbl <- data.frame(
    Dimension    = seq_len(length(pa$eig_obs)),
    Observed     = round(pa$eig_obs, 5L),
    Reference    = round(pa$eig_q,   5L),
    Exceeds_null = pa$eig_obs > pa$eig_q,
    stringsAsFactors = FALSE
  )

  out <- structure(list(
    ALSI_index      = alsi_obj$alpha,
    ALSI_z          = as.vector(scale(alsi_obj$alpha)),
    K_PA            = K,
    K_star          = K,
...
#' @export
print.alsi_binary <- function(x, ...) {
  cat("alsi_binary (MCA, variance-weighted L2 norm)\n")
  cat(sprintf("  K_PA = K* = %d  |  N = %d\n", x$K_star, length(x$ALSI_index)))
  cat("\nStability table:\n")
  print(x$stability_table[, c("Dimension","Eigenvalue","Angle_deg_median",
                               "Tucker_phi_median","Grade_Tucker")],
        row.names = FALSE)
  cat(sprintf("\nALSI (raw, range [%.3f, %.3f]):\n",
